...
public class VirtualWaypoint
{
    ...
    public VirtualWaypoint Clone()
    {
        VirtualWaypoint waypoint = new VirtualWaypoint();
...
[System.Serializable]
public class AutomaticVirtualPathSegment
{
    ...
    public List<VirtualWaypoint> virtual_waypoints;
    public Transform target;
    public float distanceToTarget = Mathf.Infinity;


    public AutomaticVirtualPathSegment()
    {
        virtual_waypoints = new List<VirtualWaypoint>();
    }

    public AutomaticVirtualPathSegment Clone()
    {
        AutomaticVirtualPathSegment segment = new AutomaticVirtualPathSegment();
        ...
        foreach (VirtualWaypoint waypoint in this.virtual_waypoints)
        {
    ...
    public List<float> GainsList()
    {
        ...
        foreach (VirtualWaypoint waypoint in virtual_waypoints)
        {
    ...
    public List<Vector3> Solve()
    {
        ...
        for (int i=1; i<forward.Count; i++)
        {
            ...
            if (resets && !InBounds(forward[i]))
            {
                ...
                {
                    VirtualWaypoint tmp_waypoint = virtual_waypoints[j].Clone();
...
{
    [SerializeField]
    private List<AutomaticVirtualPathSegment> virtualPathSegments;
    public List<AutomaticVirtualPathSegment> VirtualPathSegments {get; set; }

    void Awake()
    {
        virtualPathSegments = new List<AutomaticVirtualPathSegment>();
    }
    ...
    void DrawPhysical()
    {
        for (int i = 0; i < virtualPathSegments.Count; i++)
        {
            AutomaticVirtualPathSegment segment = virtualPathSegments[i];
            List<Vector3> physical_waypoints = segment.Solve();
    ...
    public void DrawVirtualPath()
    {
        ...
        if (virtualPathSegments.Count >= 2)
        {
            for (int i = 0; i < virtualPathSegments.Count - 1; i++)
            {
                List<VirtualWaypoint> waypoints0 = virtualPathSegments[i].virtual_waypoints;
                List<VirtualWaypoint> waypoints1 = virtualPathSegments[i + 1].virtual_waypoints;
                    ...
                    waypoints1[0].transform != null)
                {
                    Debug.DrawLine(waypoints0[waypoints0.Count - 1].transform.position,
                                   ...
                                   virtualPathSegments[i].virtualPathColor);
                }
            }
        }
    }
    ...
    {
        virtualPathSegments.Insert(index, new AutomaticVirtualPathSegment());
    }

    public void RemovePathSegment(int index)
    {
        virtualPathSegments.RemoveAt(index);
    }

    public void SwapPathSegments(int i, int j)
    {
        AutomaticVirtualPathSegment tmp = virtualPathSegments[i];
        virtualPathSegments[i] = virtualPathSegments[j];
        virtualPathSegments[j] = tmp;
    }
}
